...
def cluster_data(train_groups_images, s):
    ...
    for i, shard in enumerate(shards):
        print(f"The {i+1}th shard inlcuding:", shard)
...
"""CAUSE-UCDP Ends"""
accu = []
eoi = np.arange(0,11,10)
for e in range(1,11):
    model.fit(selected_train_images, selected_train_labels, batch_size= 256,epochs=1, validation_split=0.1, verbose=2) # For CAUSE UCDP
    if e in eoi:
...
model.compile(optimizer='adam',
            loss=tf.keras.losses.SparseCategoricalCrossentropy(from_logits=True),
            metrics=['accuracy'])
epochs_of_interest = np.arange(0,55,10)

for epoch in range(1, 11):
    model.fit(train_images_split[0], train_labels_split[0],validation_split=0.1, epochs=1,batch_size= 128,callbacks=callbacks,verbose=2)
    if epoch in epochs_of_interest:
        _, acc = model.evaluate(test_images, test_labels, verbose=0)
        accuracy.append(acc)

print('test-baselines:', accuracy)